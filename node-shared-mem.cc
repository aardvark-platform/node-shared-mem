...
SharedMemory::SharedMemory(const Napi::CallbackInfo& info) : Napi::ObjectWrap<SharedMemory>(info)
{
	...
	auto buffer = Napi::ArrayBuffer::New(env, data, (size_t)len);
...
Napi::Value SharedMemory::Close(const Napi::CallbackInfo& info) {
	...
	auto nameValue = this->Value().Get("name");
	...
	if(this->ptr != nullptr) {
		if(!UnmapViewOfFile(this->ptr)) {
			...
			failv("[SharedMemory] could not unmap \"%s\" (ERROR: %s)", name.c_str(), err.c_str());
		}
		this->ptr = nullptr;
	}
	...
	if(this->handle != nullptr) {
		if(!CloseHandle(this->handle)) {
			auto err = GetLastErrorAsString();
			failv("[SharedMemory] could not close \"%s\" (ERROR: %s)", name.c_str(), err.c_str());
		}
		this->handle = nullptr;
	}
#else
	...
	if(this->ptr != nullptr) {
		if(munmap(this->ptr, this->length) != 0) {
			close(this->handle);
			failv("[SharedMemory] could not unmap \"%s\" (ERROR: %s)", name.c_str(), strerror(errno));
		}
		this->ptr = nullptr;
	}
	if(this->handle != 0) {
		if(close(this->handle) != 0) {
			failv("[SharedMemory] could not close \"%s\" (ERROR: %s)", name.c_str(), strerror(errno));
		}
		this->handle = 0;
	}
#endif
